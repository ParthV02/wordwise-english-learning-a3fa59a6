...
import { CheckCircle2, XCircle, RotateCcw, Newspaper } from "lucide-react";
...
export default function QuizPage() {
  ...
  if (wordBankCount < 5) {
    return (
      <div className="container py-16 text-center space-y-4 fade-in">
        ...
        <p className="text-muted-foreground">You need at least 5 words in your word bank to take the quiz.</p>
        <Link to="/news">
  ...
  const handleRestart = () => { setCurrentQ(0); setSelected(null); setAnswers([]); setFinished(false); };
      ...
      <div className="container py-8 max-w-2xl space-y-8 fade-in">
        <div className="text-center space-y-6">
          ...
          <div className="flex justify-center gap-6">
            ...
            <div className="text-center"><p className="text-2xl font-bold text-destructive">{quizQuestions.length - correctCount}</p><p className="text-sm text-muted-foreground">Incorrect</p></div>
          </div>
        </div>
